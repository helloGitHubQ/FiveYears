...
- if（判断）
  ...
  1、给 where 后面加上 1=1 ，以后条件都是 and XXX

  2、mybatis 使用 where 标签来将所有的查询条件包括在内。

  ​		where 只会去掉一个 and 或者 or 
...
<!--
	collection：指定要遍历的集合
		list类型的参数会特殊处理封装在 map 中，map 的 key 就叫 list
	item：将当前遍历出的元素赋值给指定的变量
	index：索引。（遍历 list 的时候 index 是索引,item 就是当前值；遍历 map 的时候 index 表示 map 中的 key ,item就是 map 的值）
...
​		1、  可以用 foreach 遍历  mysql支持values(),(),()语法
